...
def check_first_edit_summaries(pages_input):
    ...
    with open(pages_input, 'r') as pages_info:
        ...
        for line in pages_info.readlines():
            ...
            #count_pages += 1
    pages_info.close()
    total_pages = ((len(id_status), count_translated, count_translated/float(len(id_status)), count_not_empty, count_translated/float(count_not_empty)))
